module es {
    ...
    export class FollowCamera extends Component implements IUpdatable {
        ...
        public update() {
            let halfScreen = this.camera.bounds.size.multiplyScaler(0.5);
            ...
            this.camera.transform.position = Vector2.lerp(this.camera.position, Vector2.add(this.camera.position, this._desiredPositionDelta), this.followLerp);
        ...
        public clampToMapSize(position: Vector2) {
            let halfScreen = this.camera.bounds.size.multiplyScaler(0.5).add(new Vector2(this.mapSize.x, this.mapSize.y));
