module es {
    ...
    export class Input {
        ...
        private static touchBegin(touch: Touch, event: TouchEvent) {
        ...
        private static touchMove(touch: Touch, event: TouchEvent) {
            if (touch.identifier == this._gameTouchs[0].touchPoint) {
                this.setpreviousTouchState(this._gameTouchs[0]);
            }
    
            let touchIndex = this._gameTouchs.findIndex(t => t.touchPoint == touch.identifier);
            if (touchIndex != -1) {
        ...
        private static touchEnd(touch: Touch, event: TouchEvent) {
            let touchIndex = this._gameTouchs.findIndex(t => t.touchPoint == touch.identifier);
            if (touchIndex != -1) {
                let touchData = this._gameTouchs[touchIndex];
                touchData.reset();
